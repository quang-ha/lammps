#include <math.h>
#include <stdlib.h>
#include "pair_sph_surfacereaction_Langmuir_porous_simple_2.h"
...
PairSPHSurfaceReactionLangmuirPorousSimple2::PairSPHSurfaceReactionLangmuirPorousSimple2(LAMMPS *lmp) : Pair(lmp)
{
  restartinfo = 0;
  ...
  if (ixA < 0)
    error->all(FLERR,
               "Can't find property xA for pair_style sph/surfacereaction/Langmuir/porous/simple/2");
  xA = atom->dvector[ixA];
  ...
  if (idxA < 0)
    error->all(FLERR,
               "Can't find property dxA for pair_style sph/surfacereaction/Langmuir/porous/simple/2");
  ...
  if (iyA < 0)
    error->all(FLERR,
               "Can't find property yA for pair_style sph/surfacereaction/Langmuir/porous/simple/2");
  yA = atom->dvector[iyA];
  ...
  if (idyA < 0)
    error->all(FLERR,
               "Can't find property dyA for pair_style sph/surfacereaction/Langmuir/porous/simple/2");
  ...
  if (iDA < 0)
    error->all(FLERR,
               "Can't find property DA for pair_style sph/surfacereaction/Langmuir/porous/simple/2");
  DA = atom->dvector[iDA];
...
PairSPHSurfaceReactionLangmuirPorousSimple2::~PairSPHSurfaceReactionLangmuirPorousSimple2() {
  if (allocated) {
...
/* ---------------------------------------------------------------------- */
void PairSPHSurfaceReactionLangmuirPorousSimple2::init_style()
{
...
void PairSPHSurfaceReactionLangmuirPorousSimple2::compute(int eflag, int vflag) {
  ...
  for (ii = 0; ii < inum; ii++) {
	...
	if ((is_periodic==1) || (not (x[j][0] < domain->boxlo[0] || x[j][0] > domain->boxhi[0] ||
      ...
      if (itype==2) {
        ...
        dxA[i] -= kaA*micro_pore*(xA[i] - xAeq);
        dyA[i] += kaA*micro_pore*(xA[i] - xAeq);
      }
...
void PairSPHSurfaceReactionLangmuirPorousSimple2::allocate() {
  allocated = 1;
  int n = atom->ntypes;
...
void PairSPHSurfaceReactionLangmuirPorousSimple2::settings(int narg, char **arg) {
  if (narg != 0)
    error->all(FLERR,
               "Illegal number of setting arguments for pair_style sph/surfacereaction/Langmuir/porous/simple/2");
}
...
void PairSPHSurfaceReactionLangmuirPorousSimple2::coeff(int narg, char **arg) {
  if (narg != 8)
    error->all(FLERR,"Incorrect number of args for pair_style sph/surfacereaction/Langmuir/porous/simple/2 coefficients");
  if (!allocated)
    allocate();
  ...
  xAeq = force->numeric(FLERR,arg[5]);
...
double PairSPHSurfaceReactionLangmuirPorousSimple2::init_one(int i, int j) {

  if (setflag[i][j] == 0) {
    error->all(FLERR,"All pair sph/surfacereaction/Langmuir/porous/simple/2 coeffs are not set");
  }
...
double PairSPHSurfaceReactionLangmuirPorousSimple2::single(int i, int j, int itype, int jtype,
...
int PairSPHSurfaceReactionLangmuirPorousSimple2::pack_forward_comm(int n, int *list, double *buf,
...
void PairSPHSurfaceReactionLangmuirPorousSimple2::unpack_forward_comm(int n, int first, double *buf)
{
  int i, m, last;
...
int PairSPHSurfaceReactionLangmuirPorousSimple2::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void PairSPHSurfaceReactionLangmuirPorousSimple2::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
